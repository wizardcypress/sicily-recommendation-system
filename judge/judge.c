...
void update_user(int solved)
{
    ...
    if (!fetch_row() && solved)
    {
        query("UPDATE user SET solved=solved+1 WHERE uid='%lu'", user_id);
    }
...
int diff(char *file1, char *file2, int exact_match)
{
    ...
    snprintf(cmd, sizeof(cmd), "diff %s %s %s > %s/diff.result", exact_match ? "" : "-B -b", file1, file2, WORKDIR);
    system(cmd);
